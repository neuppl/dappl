...
let conj (b : bexpr) (r : string list) ?(neg = false) =
  if neg then
...
| Ite(g, thn, els)  ->  (match g with
                        | Ident s ->
                          ...
                          let unn_t'  = conj (conj unn_t l_t) l_e ~neg:true in
                          let unn_e'  = conj (conj unn_e l_e) l_t ~neg:true in
...
| ChooseWith(x, l)  ->  (match x with
                        | Ident s ->
                          ...
                          let phis = zip_with_fn phis rews_and_rews_to_rm
                            (fun p (r, nr) -> conj (conj p r) nr ~neg:true) in
